...
f2 = Flagellum(PlanarFlagellum(1., 0., 0.6, 0., 2π, 2π, 2π, 0.0), N, Q) 
...
lines(tprob.traj.x)
viz(tprob) # move the slider along
# For a periodic trajectory you can continue the trajectory by adding copies onto the end

new_traj = continue_periodic_trajectory(tprob.traj, 10) # continue for 10 periods
...
new_traj2 = continue_periodic_trajectory(tprob2.traj, 20)
animate(new_traj2, flg)
